...
static void FW_MODE_RebuildView(void)
{
    ...
    s_fw_mode_rt.ui_view.package_ready  = s_fw_mode_rt.scan_result.header_valid;
...
void FW_MODE_RunFromReset(void)
{
    ...
    FW_SD_ScanUpdatePackage(&s_fw_mode_rt.scan_result);
    app_valid = (FW_FLASH_IsAppVectorValid() != false) ? 1u : 0u;

    /* ---------------------------------------------------------------------- */
    /*  진입 우선순위                                                           */
    /*  1) 사용자 chord                                                        */
    /*  2) SD package 자동 업데이트 (단, 설치된 버전과 다를 때만)             */
    /*  3) app invalid / fault / IWDG 3회                                      */
    /*  4) 그 외에는 app jump                                                  */
    /* ---------------------------------------------------------------------- */
    if (user_entry != 0u)
    {
    ...
    else if ((app_valid == 0u) ||
             (fault_present != 0u) ||
             (s_fw_mode_rt.boot_ctl.iwdg_unconfirmed_reset_count >= FW_BOOT_AUTOFAIL_THRESHOLD))
    {
        s_fw_mode_rt.entry_reason = (app_valid == 0u) ? FW_BOOT_REASON_INVALID_APP : FW_BOOT_REASON_BOOT_FAIL;
    }
    else
    {
        FW_MODE_PrepareAndJumpToApp();
