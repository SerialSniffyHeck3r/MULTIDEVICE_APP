...
{
    /* ---------------------------------------------------------------------- */
    /* Flight UI 동적 누적치 초기화                                             */
    /*                                                                          */
    /* 공통 renderer 는 screen snapshot 은 그대로 유지하면서도,                 */
    /* 화면 표시용 n초 평균 바리오 / 평균 속도 / Top Speed 를                    */
    /* display 계층 내부 캐시에 누적한다.                                       */
    /*                                                                          */
    /* 따라서 앱 init 시점에는 이 캐시를 먼저 초기화해 두는 편이 안전하다.      */
    /* ---------------------------------------------------------------------- */
    Vario_Display_ResetDynamicMetrics();
