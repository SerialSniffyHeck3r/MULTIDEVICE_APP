...
static vario_navigation_state_t s_nav APP_CCMRAM_BSS;
...
static bool vario_nav_runtime_has_valid_gps(const vario_runtime_t *rt)
{
    ...
    /* ------------------------------------------------------------------ */
    /* navigation 怨꾩링? Vario_State 媛 ?대? 留뚮뱺 gps_valid 瑜??좊ː?섎릺,   */
    /* ?????쾿??醫뚰몴濡쒖꽌 留먯씠 ?섎뒗 踰붿쐞?몄? ??踰???諛⑹뼱?쒕떎.          */
    /*                                                                    */
    /* - ?꾨룄/寃쎈룄 踰붿쐞 outside 嫄곕?                                        */
    /* - (0,0) 嫄곕?                                                         */
    /*                                                                    */
    /* ?대젃寃??섎㈃ GPS 珥덇린???붿긽?대굹 weak fix 媛 SAVE PATH 濡?            */
    /* ?ㅼ뼱?ㅻ뒗 ?쇱쓣 以꾩씪 ???덈떎.                                         */
    /* ------------------------------------------------------------------ */
...
/*                                                                            */
/* breadcrumb trail renderer ??navigation ?대? ???援ъ“瑜?吏곸젒 李몄“?섏?       */
/* ?딅뒗?? display layer ????export API 留??ъ슜??START / LAND / WPT         */
/* marker snapshot ???산퀬, 洹?寃곌낵瑜??붾㈃ 醫뚰몴濡??ъ쁺?댁꽌 洹몃┛??             */
/*                                                                            */
/* ?대젃寃?遺꾨━?섎㈃                                                             */
/* - display 媛 persistence layout ??紐곕씪???섍퀬                             */
/* - 異뷀썑 site ???援ъ“媛 諛붾뚯뼱??display contract ???좎??????덈떎.        */
/* -------------------------------------------------------------------------- */
...
uint8_t Vario_Navigation_GetTrailMarkerCount(void)
{
    ...
    /* ------------------------------------------------------------------ */
    /* START ??launch point 瑜?export ?쒕떎.                                */
    /* trail ??媛???ㅻ옒??breadcrumb ???takeoff capture 吏?먯쓣 ?곕㈃     */
    /* ?ъ슜?먭? 湲곕??섎뒗 "異쒕컻?? 怨?????留욌뒗??                         */
    /* ------------------------------------------------------------------ */
    ...
    /* ------------------------------------------------------------------ */
    /* ?쒖꽦 SITE ??LANDABLE / WAYPOINT 瑜?紐⑤몢 overlay marker 濡??대낫?몃떎.*/
    /* active target ?섎굹留?蹂댁뿬 二쇱? ?딄퀬, 二쇰?????λ맂 field/point 瑜?   */
    /* ?④퍡 蹂댁뿬 以섏빞 breadcrumb trail page ???곹솴 ?뚯븙???ъ썙吏꾨떎.        */
    /* ------------------------------------------------------------------ */
    for (i = 0u; i < VARIO_NAV_MAX_LANDABLES; ++i)
    {
...
static void vario_nav_save_current_here(uint8_t action_row, uint32_t now_ms)
{
    ...
    /* ------------------------------------------------------------------ */
    /* ?꾩옱 ?꾩튂瑜???ν븯??紐⑤뱺 ?숈옉? usable GPS fix 瑜?怨듯넻 ?꾩젣濡??붾떎. */
    /*                                                                    */
    /* ?ъ슜?먭? ?붽뎄??UX                                                  */
    /* - GPS fix 媛 ?놁쑝硫?????쒕룄瑜?議곗슜??臾댁떆?섏? ?딅뒗??            */
    /* - Toast 濡?紐낆떆?곸쑝濡?"????μ씠 ???섎뒗吏" ?뚮젮 以??           */
    /*                                                                    */
    /* action row 蹂꾨줈 臾멸뎄瑜?遺꾨━???먮㈃,                                 */
    /* ?댄썑 "CANNOT SAVE FIELD" / "CANNOT SET HOME" 媛숈? UX 濡?         */
    /* ?몃텇?뷀븯湲곕룄 ?쎈떎.                                                  */
    /* ------------------------------------------------------------------ */
...
void Vario_Navigation_RenderSettingPage(u8g2_t *u8g2)
{
    ...
    /* ------------------------------------------------------------------ */
    /* NAV 怨꾩뿴 ?섏씠吏 ?꾩뿉 confirm overlay 媛 ????                       */
    /* ?꾨옒履?由ъ뒪?멸퉴吏 怨꾩냽 洹몃━硫?湲?먭? 以묒븰 confirm box 二쇰???       */
    /* 寃뱀퀜 蹂댁뿬 ?쒓컖?곸쑝濡?吏?遺꾪빐吏꾨떎.                                   */
    /*                                                                    */
    /* confirm ??active ???숈븞?먮뒗 NAV page ??title/subtitle 留??④린怨? */
    /* 蹂몃Ц row ??洹몃━吏 ?딅뒗??                                          */
    /*                                                                    */
    /* ?대젃寃??섎㈃ confirm box ??洹몃?濡?UI_ENGINE overlay 濡?洹몃젮吏怨?    */
    /* NAV page ???곷떒 臾몃㎘留??좎???梨?蹂몃Ц 寃뱀묠???쒓굅?쒕떎.              */
    /* ------------------------------------------------------------------ */
    if (UI_Confirm_IsVisible() != false)
    {
        return;
