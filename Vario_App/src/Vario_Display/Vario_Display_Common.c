                                                   ...
                                                   const vario_runtime_t *rt)
{
    ...
    /* ------------------------------------------------------------------ */
    /* Logisoso 는 '1' 폭이 다른 숫자보다 좁아, ALT1 큰 숫자에서                */
    /* 411/421/431 과 412/422/432 의 ones 자리 X 위치가 달라져 보인다.          */
    /*                                                                    */
    /* 이 값은 absolute viewport 좌표에 딱 박힌 고정 슬롯으로 그려서         */
    /* 각 자리수가 언제나 같은 X 에 놓이게 만든다.                          */
    /* 음수 등 비정형 문자열은 helper 내부에서 기존 renderer 로 fallback 한다. */
    /* ------------------------------------------------------------------ */
    vario_display_draw_logisoso_integer_value(u8g2,
                                              value_x,
