...
/* -------------------------------------------------------------------------- */
/* UI vario 전용 simple display filter                                        */
/*                                                                            */
...
/*                                                                            */
/* 이번 개정에서는 fast bar / audio 경로는 별도 direct fast path 를 쓰고,     */
/* 이 필터는 숫자 표시용 current vario만 담당한다.                             */
/*                                                                            */
/* 따라서 구현은                                                              */
/* - input slew clamp                                                         */
/* - single-pole EMA                                                          */
/* - zero hysteresis latch                                                    */
/* 정도만 남겨 둔 최소 구조다.                                                */
/* -------------------------------------------------------------------------- */
...
typedef struct
{
    ...
    /* legacy buffer 자리                                                      */
    /*                                                                        */
    /* 과거 robust filter 구현과의 source compatibility를 위해 남겨 둔다.      */
    /* 현재 simple filter의 핵심 동작은                                        */
    /* robust_input_mps / smoothed_mps / display_mps 에 집중되어 있다.         */
    /* ---------------------------------------------------------------------- */
    uint8_t input_count;
    uint8_t input_head;
...
/*                                                                            */
/* 처리 단계                                                                  */
/* 1) 단발성 급변을 완화하는 input slew clamp                                 */
/* 2) single-pole EMA                                                         */
/* 3) 0 근처 chatter 제거용 hysteresis zero latch                             */
/*                                                                            */
/* damping_level / average_seconds 는 기존 QuickSet UI 와 의미를 맞추되,      */
/* 여러 단계에 중복 반영하지 않고 이 한 함수 안에서만 사용한다.               */
/* -------------------------------------------------------------------------- */
