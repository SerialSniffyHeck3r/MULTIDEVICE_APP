...
#ifndef MOTOR_MAIN_SPLIT_GAP_X
#define MOTOR_MAIN_SPLIT_GAP_X            8
#endif
...
static uint8_t motor_main_get_top_major_interval_count(const motor_state_t *state,
                                                       motor_main_top_mode_t mode)
{
    if (mode == MOTOR_MAIN_TOP_MODE_RPM)
    {
        return (uint8_t)(motor_main_get_rpm_scale_max(state) / 2000);
    }
                                               ...
                                               uint8_t major_intervals)
{
    ...
    if ((u8g2 == 0) || (w < 20) || (h <= MOTOR_MAIN_TOP_GAUGE_BAR_H) || (max_value <= 0) || (major_intervals == 0u))
    {
        return;
    }

    value = motor_main_clamp_i32(value, 0, max_value);
    track_y = (int16_t)(y + h - MOTOR_MAIN_TOP_GAUGE_BAR_H);
    inner_w = (int16_t)(w - 2);
    fill_w = (int16_t)((value * inner_w) / max_value);

    for (i = 0u; i <= major_intervals; i++)
    {
        int16_t tick_x;

        tick_x = (int16_t)(x + (((int32_t)i * (w - 1)) / major_intervals));
        u8g2_DrawVLine(u8g2, tick_x, y, MOTOR_MAIN_GAUGE_MAJOR_TICK_H);
    }
    ...
    for (i = 0u; i < major_intervals; i++)
    {
        ...
        tick_x = (int16_t)(x + ((((int32_t)i * 2) + 1) * (w - 1)) / ((int32_t)major_intervals * 2));
        u8g2_DrawVLine(u8g2, tick_x, y, MOTOR_MAIN_GAUGE_MINOR_TICK_H);
    }

    u8g2_DrawFrame(u8g2, x, track_y, w, MOTOR_MAIN_TOP_GAUGE_BAR_H);
    if (fill_w > 0)
    {
        u8g2_DrawBox(u8g2,
                     (int16_t)(x + 1),
                     (int16_t)(track_y + 1),
                     (uint8_t)fill_w,
                     (uint8_t)(MOTOR_MAIN_TOP_GAUGE_BAR_H - 2));
    }
}
                                          ...
                                          int32_t minor_step)
{
    ...
    if (minor_step > 0)
    {
        for (step = minor_step; step < max_abs_value; step += minor_step)
        {
            ...
            u8g2_DrawVLine(u8g2, (int16_t)(center_x - offset_px), y, MOTOR_MAIN_GAUGE_MINOR_TICK_H);
            u8g2_DrawVLine(u8g2, (int16_t)(center_x + offset_px), y, MOTOR_MAIN_GAUGE_MINOR_TICK_H);
        }
    }
    ...
    if (major_step > 0)
    {
        for (step = major_step; step <= max_abs_value; step += major_step)
        {
            ...
            offset_px = (int16_t)((step * (int32_t)(w - 1)) / (2 * max_abs_value));
            u8g2_DrawVLine(u8g2, (int16_t)(center_x - offset_px), y, MOTOR_MAIN_GAUGE_MAJOR_TICK_H);
            u8g2_DrawVLine(u8g2, (int16_t)(center_x + offset_px), y, MOTOR_MAIN_GAUGE_MAJOR_TICK_H);
        }
    }
                                        ...
                                        const motor_state_t *state)
{
    ...
    major_intervals = motor_main_get_top_major_interval_count(state, mode);
                                          ...
                                          const motor_state_t *state)
{
    ...
    speed_x10 = Motor_Units_ConvertSpeedX10((int32_t)state->nav.speed_kmh_x10, &state->settings.units);
    max_speed_x10 = Motor_Units_ConvertSpeedX10((int32_t)state->session.max_speed_kmh_x10, &state->settings.units);
    speed_unit = Motor_Units_GetSpeedSuffix(&state->settings.units);
    ...
    motor_main_format_speed_integer(speed_buf, sizeof(speed_buf), speed_x10);
    (void)snprintf(max_buf, sizeof(max_buf), "MAX %ld", (long)(max_speed_x10 / 10));
                              ...
                              const motor_state_t *state)
{
    ...
    int16_t content_y;
    int16_t content_h;
    int16_t split_left_w;
    int16_t split_right_x;
    int16_t split_right_w;
    ...
    if (viewport->h < (MOTOR_MAIN_TOP_GAUGE_H + 56))
    {
    ...
    gauge_y = outer_y;
    content_y = (int16_t)(gauge_y + MOTOR_MAIN_TOP_GAUGE_H + 3);
    content_h = (int16_t)(viewport->y + viewport->h - content_y);

    split_left_w = (int16_t)((outer_w - MOTOR_MAIN_SPLIT_GAP_X) / 2);
    split_right_x = (int16_t)(outer_x + split_left_w + MOTOR_MAIN_SPLIT_GAP_X);
    split_right_w = (int16_t)(outer_x + outer_w - split_right_x);

    motor_main_draw_top_section(u8g2,
                                outer_x,
                                gauge_y,
                                outer_w,
    ...
    motor_main_draw_left_section(u8g2,
                                 outer_x,
                                 content_y,
                                 split_left_w,
                                 content_h,
    ...
    motor_main_draw_right_section(u8g2,
                                  split_right_x,
                                  content_y,
                                  split_right_w,
                                  content_h,
