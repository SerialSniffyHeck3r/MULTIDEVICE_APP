...
/* -------------------------------------------------------------------------- */
/*  내부 helper: initial MSP가 main SRAM 범위 안인지 확인                     */
/*                                                                            */
/*  중요한 점                                                                  */
/*  - CubeIDE 기본 linker script는 _estack 를 RAM 끝 주소 자체로 둔다.       */
/*  - 즉 STM32F407VG 의 경우 흔히 0x20020000 이 첫 MSP 값으로 들어간다.      */
/*  - 따라서 상한 비교는 '< top' 이 아니라 '<= top' 이어야 한다.             */
/* -------------------------------------------------------------------------- */
static bool FW_FLASH_IsAddressInMainSram(uint32_t address)
{
    return ((address >= 0x20000000u) && (address <= 0x20020000u)) ? true : false;
}

/* -------------------------------------------------------------------------- */
/*  내부 helper: initial MSP가 CCM SRAM 범위 안인지 확인                      */
/* -------------------------------------------------------------------------- */
static bool FW_FLASH_IsAddressInCcmRam(uint32_t address)
{
    return ((address >= 0x10000000u) && (address <= 0x10010000u)) ? true : false;
}

/* -------------------------------------------------------------------------- */
/*  내부 helper: vector table의 초기 stack pointer 유효성 검사                */
/* -------------------------------------------------------------------------- */
static bool FW_FLASH_IsValidInitialStackPointer(uint32_t app_sp)
{
    if ((app_sp & 0x3u) != 0u)
    {
        return false;
    ...
    if (FW_FLASH_IsAddressInMainSram(app_sp) != false)
    {
        return true;
    }

    if (FW_FLASH_IsAddressInCcmRam(app_sp) != false)
    {
        return true;
    }
...
/* -------------------------------------------------------------------------- */
/*  내부 helper: reset handler 주소 유효성 검사                               */
/*                                                                            */
/*  Thumb bit는 1이어야 하므로, 실제 범위 비교는 bit0를 제거한 값으로 한다.   */
/* -------------------------------------------------------------------------- */
static bool FW_FLASH_IsValidResetHandlerAddress(uint32_t app_pc)
{
    uint32_t pc_even;
    ...
    if (pc_even < FW_APP_BASE_ADDRESS)
    {
        return false;
    }

    if (pc_even >= FW_FLASH_END_ADDRESS)
    {
        return false;
    }
...
bool FW_FLASH_IsAppVectorValid(void)
{
    ...
    if (FW_FLASH_IsValidInitialStackPointer(app_sp) == false)
    {
        return false;
    }

    if (FW_FLASH_IsValidResetHandlerAddress(app_pc) == false)
    {
        return false;
    }
...
/*                                                                            */
/*  중요한 점                                                                  */
/*  - bootloader가 jump 전에 __disable_irq() 를 호출하는 것은 맞다.            */
/*  - 하지만 그 뒤 app Reset_Handler 를 호출하기 전에 PRIMASK 를 반드시      */
/*    다시 0으로 풀어 줘야, app가 reset 직후와 같은 인터럽트 상태를 받는다.   */
/*  - 이 처리가 없으면 SysTick / TIM / DMA / UART interrupt가                */
/*    전부 막힌 상태로 app가 시작할 수 있다.                                  */
/* -------------------------------------------------------------------------- */
void FW_FLASH_JumpToApp(void)
{
    ...
    if (FW_FLASH_IsAppVectorValid() == false)
    {
        return;
    }
...
#if defined(SCB_ICSR_PENDSTCLR_Msk) && defined(SCB_ICSR_PENDSVCLR_Msk)
                                               ...
                                               void *user_context)
{
    ...
    /* ---------------------------------------------------------------------- */
    /*  CRC만 맞고 vector table이 잘못된 경우를 여기서 한 번 더 걸러낸다.    */
    /*  예를 들어 잘못된 base로 link한 image를 포장해도,                      */
    /*  CRC는 맞아버릴 수 있으므로 이 검사가 필요하다.                        */
    /* ---------------------------------------------------------------------- */
    if (FW_FLASH_IsAppVectorValid() == false)
    {
        return HAL_ERROR;
