                                         ...
                                         const vario_gc_polar_t *polar)
{
    ...
    search_min_mps = polar->v1_mps * 0.90f;
    if (search_min_mps < 5.0f)
    {
        search_min_mps = 5.0f;
    }

    search_max_mps = polar->v3_mps + 6.0f;
    if (search_max_mps < (polar->v3_mps * 1.15f))
    {
        search_max_mps = polar->v3_mps * 1.15f;
    }
...
static void vario_gc_update_estimated_te(vario_runtime_t *rt, uint32_t now_ms)
{
    ...
    else
    {
        ...
        /*  TE variometer의 기본식                                            */
        /*      dh/dt + v/g * dv/dt                                           */
        /*  를 쓰되, 여기서 v는 pitot TAS가 아니라                            */
        /*  GPS ground vector - circling wind vector 로 복원한 추정 airspeed다.*/
        /*  따라서 값은 "true TE" 가 아니라 pilot cue 용 estimated TE다.      */
        /* ------------------------------------------------------------------ */
