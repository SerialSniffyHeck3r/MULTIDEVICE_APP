                                                       ...
                                                       uint8_t average_seconds)
{
    ...
    /* ------------------------------------------------------------------ */
    /*  기존 값보다 quiet tau를 약간 낮춰                                       */
    /*  climb/sink 시작 응답을 살짝 더 빠르게 만든다.                           */
    /*  단, damping/average seconds 가 커질수록 다시 충분히 무거워진다.         */
    /* ------------------------------------------------------------------ */
    return 0.16f + (average_norm * 0.16f) + (damping_norm * 0.14f);
}
                                                          ...
                                                          float innovation_mps)
{
    ...
    {
        tau_s = quiet_tau_s * 0.14f;
    }
    else if (abs_innovation_mps >= 0.60f)
    {
        tau_s = quiet_tau_s * 0.22f;
    }
    else if (abs_innovation_mps >= 0.25f)
    {
        tau_s = quiet_tau_s * 0.38f;
    }
    else if (abs_innovation_mps >= 0.12f)
    {
        tau_s = quiet_tau_s * 0.55f;
    }

    return vario_ui_vario_filter_clampf(tau_s, 0.045f, quiet_tau_s);
}
                                                         ...
                                                         uint8_t average_seconds)
{
    ...
    /* ------------------------------------------------------------------ */
    /*  0 부근 latch는 유지하되 문턱을 조금 낮춰                               */
    /*  climb/sink 시작이 숫자에 더 빨리 반영되게 한다.                         */
    /* ------------------------------------------------------------------ */
    enter_mps = 0.04f + (average_norm * 0.02f) + (damping_norm * 0.03f);
    exit_mps = enter_mps + 0.05f;
