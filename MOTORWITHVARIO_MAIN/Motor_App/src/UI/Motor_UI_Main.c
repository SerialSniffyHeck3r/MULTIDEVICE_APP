...
void Motor_UI_DrawScreen_Main(u8g2_t *u8g2, const ui_rect_t *viewport, const motor_state_t *state)
{
    ...
    /* ---------------------------------------------------------------------- */
    /*  메인 계기판 화면                                                       */
    /*  - viewport 전체를 계기판처럼 꽉 쓰는 screen                            */
    /*  - 좌상단 속도 / 우상단 bank                                             */
    /*  - 중단 lateral G / accel-brake G                                       */
    /*  - 하단 altitude / grade / trip mini panel                              */
    /* ---------------------------------------------------------------------- */
    top_y = viewport->y + 2;
    ...
    Motor_UI_DrawLabeledValueBox(u8g2, viewport->x + 4,   top_y,      114, 44, "SPEED",   speed_text, 1u);
    Motor_UI_DrawLabeledValueBox(u8g2, viewport->x + 122, top_y,      114, 44, "BANK",    bank_text,  1u);
    Motor_UI_DrawLabeledValueBox(u8g2, viewport->x + 4,   top_y + 48, 114, 28, "LAT G",   latg_text,  0u);
    Motor_UI_DrawLabeledValueBox(u8g2, viewport->x + 122, top_y + 48, 114, 28, "ACC/BRK", long_text,  0u);

    Motor_UI_DrawLabeledValueBox(u8g2, viewport->x + 4,   top_y + 78, 74,  22, "ALT",     alt_text,   0u);
    Motor_UI_DrawLabeledValueBox(u8g2, viewport->x + 82,  top_y + 78, 74,  22, "GRADE",   grade_text, 0u);
