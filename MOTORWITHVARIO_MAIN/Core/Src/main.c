...
int main(void)
{
  ...
  /* ------------------------------------------------------------------------ */
  /* 부팅 시 F3 눌림으로 UI boot profile 을 latch 한다.                        */
  /*                                                                          */
  /* 요구 동작                                                                  */
  /* - F3 을 누르지 않고 부팅 : VARIO root 로 정상 진입                         */
  /* - F3 을 누른 채 부팅      : 기존 TEST / DEBUG / GPS legacy profile 진입    */
  /*                                                                          */
  /* 중요한 이유                                                                 */
  /* - Button_Init() 는 부팅 시점의 현재 안정 버튼 상태를 baseline 으로 읽는다. */
  /* - 따라서 바로 다음 줄에서 Button_IsPressed(BUTTON_ID_3) 를 검사하면        */
  /*   "부팅 순간 F3 hold" 를 가장 안전하게 latch 할 수 있다.                   */
  /* - 이 latch 결과는 UI_Engine_Init() 이전에 넣어야, 엔진 기본 root screen 이  */
  /*   처음부터 올바르게 결정된다.                                              */
  /* ------------------------------------------------------------------------ */
  if (Button_IsPressed(BUTTON_ID_3) != false)
  {
  ...
  /* ---------------------------------------------------------------------- */
  POWER_STATE_EnterPowerOnConfirm(HAL_GetTick());
  ...
  while (POWER_STATE_IsUiBlocking() != false)
  {
      ...
      /* ------------------------------------------------------------------ */
      POWER_STATE_TaskBootGate(power_gate_now_ms);
  }
