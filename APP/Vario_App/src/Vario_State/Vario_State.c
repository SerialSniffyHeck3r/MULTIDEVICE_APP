...
static struct
{
    ...
    /*                                                                        */
    /*  last_fast_present_ms : 20Hz vario gauge publish cadence                */
    ...
    /* ---------------------------------------------------------------------- */
    /*  ALT3 contextual anchors                                                */
    /*                                                                        */
    /*  ALT3는 누적 gain / thermal gain / arrival height 를 같은 슬롯에       */
    /*  번갈아 싣는다. thermal gain은 sink 구간이나 더 낮은 저점이 나오면      */
    /*  기준점을 자동으로 아래로 재설정해서 "이번 thermal에서 얼마나 올랐는지" */
    /*  를 보여 준다.                                                          */
    /* ---------------------------------------------------------------------- */
    bool    thermal_reference_valid;
    float   thermal_reference_altitude_m;
                                             ...
                                             const vario_settings_t *settings)
{
    ...
    imu_assist_auto = (settings != NULL) &&
                      (settings->imu_assist_mode == VARIO_IMU_ASSIST_AUTO);
    ...
    /* ---------------------------------------------------------------------- */
    if ((alt->initialized == false) || (imu_assist_auto == false))
    {
                                                ...
                                                int32_t *out_altitude_cm)
{
    ...
    /* ---------------------------------------------------------------------- */
    /* ALT1 primary altitude는 사용자 source 선택으로 바꾸지 않는다.           */
    /*                                                                        */
    /* 조종석 primary absolute altitude는                                     */
    /* - manual QNH를 기준으로 계산된                                           */
    /* - pure barometric path                                                   */
    /* - UI presentation용으로 안정화된                                         */
    /* APP_ALTITUDE.alt_display_cm 하나로 고정한다.                            */
    /*                                                                        */
    /* raw QNH / fused / GPS altitude는 debug/secondary slot 용으로만          */
    /* 남기고, Alt1 selection에는 관여시키지 않는다.                           */
    /* ---------------------------------------------------------------------- */
...
static void vario_state_update_display_filter(uint32_t now_ms)
{
    ...
    source_changed = (s_vario_state.altitude_source_tracking_valid == false) ||
                     (VARIO_ALT_SOURCE_DISPLAY != s_vario_state.last_altitude_source);
    altitude_jump = false;
    ...
    if ((s_vario_state.last_fast_present_ms == 0u) ||
        ((filter_timestamp_ms - s_vario_state.last_fast_present_ms) >=
         VARIO_STATE_FAST_PRESENT_PERIOD_MS))
    {
        ...
        /*  중요                                                               */
        /*                                                                    */
        /*  fast_vario_bar_mps staging 값만 20Hz로 바꿔서는 LCD가 20Hz로       */
        /*  보장되지 않는다. 현재 VARIO app은 상태머신이 매 루프 redraw를      */
        /*  강제하지 않고, UI engine은 redraw_request를 본다.                 */
        /*                                                                    */
        /*  따라서 fast gauge cadence를 20Hz로 올릴 때는 fast path snapshot   */
        /*  갱신 시점에도 redraw_request를 함께 세워야 실제 게이지가 20Hz로   */
        /*  따라온다.                                                          */
        /*                                                                    */
        /*  설정/메뉴 페이지까지 20Hz로 계속 repaint할 필요는 없으므로         */
        /*  메인 flight screen 계열에서만 redraw를 올린다.                    */
        /* ------------------------------------------------------------------ */
        if ((s_vario_state.current_mode == VARIO_MODE_SCREEN_1) ||
            (s_vario_state.current_mode == VARIO_MODE_SCREEN_2) ||
            (s_vario_state.current_mode == VARIO_MODE_SCREEN_3))
        {
...

{
            ...
            if (takeoff_candidate_active != false)
            {
                ...
                else if (s_vario_state.takeoff_anchor_valid != false)
                {
                          ...
                          (s_vario_state.runtime.filtered_ground_speed_kmh >= soft_start_speed_kmh))))
                    {
                        ...
                        s_vario_state.runtime.alt3_thermal_gain_m = 0.0f;
                        ...
                        s_vario_state.thermal_reference_valid = true;
                        s_vario_state.thermal_reference_altitude_m = s_vario_state.runtime.filtered_altitude_m;
    ...
    if (s_vario_state.runtime.flight_active != false)
    {
        ...
        if (s_vario_state.thermal_reference_valid == false)
        {
            s_vario_state.thermal_reference_valid = true;
            s_vario_state.thermal_reference_altitude_m = s_vario_state.runtime.filtered_altitude_m;
        }

        if ((s_vario_state.runtime.filtered_vario_mps <= 0.0f) ||
            (s_vario_state.runtime.filtered_altitude_m < s_vario_state.thermal_reference_altitude_m))
        {
            s_vario_state.thermal_reference_altitude_m = s_vario_state.runtime.filtered_altitude_m;
        }

        s_vario_state.runtime.alt3_thermal_gain_m =
            s_vario_state.runtime.filtered_altitude_m - s_vario_state.thermal_reference_altitude_m;
        if (s_vario_state.runtime.alt3_thermal_gain_m < 0.0f)
        {
            s_vario_state.runtime.alt3_thermal_gain_m = 0.0f;
        }
    ...
    else
    {
        ...
        {
            s_vario_state.thermal_reference_valid = false;
            s_vario_state.thermal_reference_altitude_m = 0.0f;
        }
    }
}
...
void Vario_State_ResetAccumulatedGain(void)
{
    ...
    s_vario_state.runtime.max_top_vario_mps = 0.0f;
    s_vario_state.runtime.last_accum_altitude_m = s_vario_state.runtime.filtered_altitude_m;
    s_vario_state.thermal_reference_valid = (s_vario_state.runtime.derived_valid != false) ? true : false;
    s_vario_state.thermal_reference_altitude_m = s_vario_state.runtime.filtered_altitude_m;
    s_vario_state.redraw_request = 1u;
}
