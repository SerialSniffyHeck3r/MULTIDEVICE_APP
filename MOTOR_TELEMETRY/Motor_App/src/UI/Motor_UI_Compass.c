...
static void motor_ui_compass_lookup_trig(int16_t angle_deg, float *out_sin, float *out_cos)
{
    ...
    {
        /* ------------------------------------------------------------------ */
        /*  Compass heading trig LUT                                           */
        /*                                                                    */
        /*  This screen refreshes at a higher cadence than menu/settings and   */
        /*  used to call sinf/cosf every frame even though heading is already  */
        /*  quantized to whole degrees here. A one-time 360-step LUT keeps     */
        /*  the display path deterministic and avoids repeated libm work.      */
        /* ------------------------------------------------------------------ */
        for (i = 0; i < 360; ++i)
        {
            float rad;
...
void Motor_UI_DrawScreen_Compass(u8g2_t *u8g2, const ui_rect_t *viewport, const motor_state_t *state)
{
    ...
    motor_ui_compass_lookup_trig(angle_deg, &sin_v, &cos_v);
    tip_x = (int16_t)(center_x + (int16_t)((float)(radius - 6) * sin_v));
    tip_y = (int16_t)(center_y - (int16_t)((float)(radius - 6) * cos_v));
