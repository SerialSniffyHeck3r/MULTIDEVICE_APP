...
static const motor_screen_t s_home_cycle_screens[] =
{
    MOTOR_SCREEN_MAIN,
    MOTOR_SCREEN_BREADCRUMB,
    MOTOR_SCREEN_HORIZON,
...
static const motor_screen_t s_data_field_cycle_screens[] =
{
    MOTOR_SCREEN_DATA_FIELD_1,
    MOTOR_SCREEN_DATA_FIELD_2
};
...
static uint8_t motor_buttons_get_subset_index(const motor_screen_t *screens, uint8_t count, motor_screen_t screen)
{
    uint8_t i;

    if ((screens == 0) || (count == 0u))
    {
        return 0u;
    }
...
static void motor_buttons_cycle_subset_screen(const motor_screen_t *screens, uint8_t count, motor_screen_t fallback_screen)
{
    motor_state_t *state;
    uint8_t idx;

    state = Motor_State_GetMutable();
    if ((state == 0) || (screens == 0) || (count == 0u))
    {
        return;
    }

    idx = motor_buttons_get_subset_index(screens, count, (motor_screen_t)state->ui.screen);
    if (idx == 0xFFu)
    {
        Motor_State_SetScreen(fallback_screen);
        return;
    }

    idx = (uint8_t)((idx + 1u) % count);
    Motor_State_SetScreen(screens[idx]);
}
...
static void motor_buttons_adjust_setting_row(int8_t delta)
{
    ...
    case MOTOR_SCREEN_SETTINGS_GPS:
        ...
        case 1u:
            ...
            break;
        case 2u:
            settings->gps.dynamic_model = motor_buttons_wrap_u8(settings->gps.dynamic_model,
...
static void motor_buttons_handle_drive(const button_event_t *ev, uint32_t now_ms)
{
    ...
    if (((motor_screen_t)state->ui.screen == MOTOR_SCREEN_BREADCRUMB) &&
        (ev->id == BUTTON_ID_3) &&
        (ev->type == BUTTON_EVENT_LONG_PRESS))
    {
        Motor_UI_Breadcrumb_ToggleHeadingUpMode();
        Motor_State_ShowToast((Motor_UI_Breadcrumb_IsHeadingUpMode() != 0u) ? "HEAD UP" : "NORTH UP", 1200u);
        return;
    }

    if ((ev->id == BUTTON_ID_3) && (ev->type == BUTTON_EVENT_LONG_PRESS))
    {
        if (Motor_UI_Main_ResetBottomPage(state) != 0u)
        {
            Motor_State_ShowToast("TRIP RESET", 1200u);
            Motor_State_RequestRedraw();
        }
        else
        {
            Motor_State_ShowToast("NO RESET", 900u);
        }
        return;
    }
        ...
        (ev->id == BUTTON_ID_4))
    {
        ...
        Motor_UI_Breadcrumb_CycleScale();
        (void)snprintf(toast_buf,
                       sizeof(toast_buf),
                       "TRAIL %um",
                       (unsigned)Motor_UI_Breadcrumb_GetScaleMeters());
        Motor_State_ShowToast(toast_buf, 1200u);
        return;
    }
    ...
    if (ev->id == BUTTON_ID_3)
    {
        ...
        page_count = Motor_UI_Main_GetBottomPageCount();
        if (page_count != 0u)
        {
            state->ui.bottom_page = (uint8_t)((state->ui.bottom_page + 1u) % page_count);
            Motor_State_ShowToast(Motor_UI_Main_GetBottomPageName(state->ui.bottom_page), 1200u);
            Motor_State_RequestRedraw();
        }
        return;
    }
...
static void motor_buttons_handle_drive_direct(const button_event_t *ev, uint32_t now_ms)
{
    motor_state_t *state;
    ...
    if ((state->ui.overlay_visible != 0u) || (state->ui.overlay_key_armed != 0u))
    {
        motor_buttons_hide_drive_overlay(state);
    }

    if ((ev->id == BUTTON_ID_1) && (ev->type == BUTTON_EVENT_LONG_PRESS))
    {
        Motor_State_SetScreen(MOTOR_SCREEN_MAIN);
        return;
    }
    ...
    if (ev->id == BUTTON_ID_1)
    {
        motor_buttons_cycle_subset_screen(s_home_cycle_screens,
                                          (uint8_t)(sizeof(s_home_cycle_screens) / sizeof(s_home_cycle_screens[0])),
                                          MOTOR_SCREEN_MAIN);
        return;
    }
    ...
    if (ev->id == BUTTON_ID_2)
    {
        motor_buttons_cycle_subset_screen(s_data_field_cycle_screens,
                                          (uint8_t)(sizeof(s_data_field_cycle_screens) / sizeof(s_data_field_cycle_screens[0])),
                                          MOTOR_SCREEN_DATA_FIELD_1);
        return;
    }
    ...
    if (ev->id == BUTTON_ID_4)
    {
        Motor_State_ShowToast("LONG PRESS TO RESET PEAK", 1200u);
        return;
    }
...
void Motor_Buttons_Task(uint32_t now_ms)
{
    ...
    while (Button_PopEvent(&ev) != false)
    {
        ...
        case MOTOR_SCREEN_VEHICLE_SUMMARY:
            motor_buttons_handle_drive_direct(&ev, now_ms);
            break;
