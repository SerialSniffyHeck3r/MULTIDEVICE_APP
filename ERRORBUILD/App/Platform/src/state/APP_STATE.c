...
static void APP_STATE_ApplyDefaultSettingsUnlocked(void)
{
    ...
    /* ---------------------------------------------------------------------- */
    /*  altitude / vario / fusion 기본 정책은                                   */
    /*  APP_ALTITUDE helper가 한 번에 채워 넣는다.                               */
    /* ---------------------------------------------------------------------- */
    APP_ALTITUDE_ApplyDefaultSettings((app_altitude_settings_t *)&g_app_state.settings.altitude);
}
...
static void APP_STATE_ResetAltitudeUnlocked(void)
{
    ...
    g_app_state.altitude.flags = 0u;
    g_app_state.altitude.output_alt_mode = g_app_state.settings.altitude.output_alt_mode;
...
void APP_STATE_CopyAltitudeSnapshot(app_altitude_state_t *dst)
{
    ...
    /* ---------------------------------------------------------------------- */
    /*  altitude slice도 main context의 service task만 갱신한다.                */
    /*  따라서 plain memcpy로 충분하다.                                        */
    /* ---------------------------------------------------------------------- */
    memcpy(dst, (const void *)&g_app_state.altitude, sizeof(*dst));
}
