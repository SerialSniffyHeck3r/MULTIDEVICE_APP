...
/* -------------------------------------------------------------------------- */
  ...
  typedef enum
  {
      ...
      APP_UI_PAGE_ALTITUDE    = 2u,
      APP_UI_PAGE_BRIGHTNESS  = 3u,
      APP_UI_PAGE_SD          = 4u,
      APP_UI_PAGE_BLUETOOTH   = 5u,
      APP_UI_PAGE_AUDIO       = 6u,
      APP_UI_PAGE_SPI_FLASH   = 7u,
      APP_UI_PAGE_CLOCK       = 8u,
  ...
  /* -------------------------------------------------------------------------- */
  /*  ALTITUDE debug page 상태                                                  */
  /*                                                                            */
  /*  이 페이지는                                                              */
  /*  - pressure/QNH/GPS/fused/relative-home altitude를 병렬로 보여주고         */
  /*  - no-IMU / IMU-assisted variometer를 동시에 비교하며                      */
  /*  - 버튼 F2~F6으로 튜닝 파라미터를 live 적용할 수 있게 만든다.               */
  /* -------------------------------------------------------------------------- */
  typedef struct
  {
      bool    active;                 /* tune mode 진입 여부                     */
      uint8_t selected_field;         /* app_altitude_tune_field_t raw           */
  } app_altitude_edit_state_t;

  static app_altitude_edit_state_t g_altitude_edit;

  /* -------------------------------------------------------------------------- */
  /*  cm -> m.xx 문자열 helper                                                  */
  /* -------------------------------------------------------------------------- */
  static void APP_FormatSignedCentimeterAsMeterText(char *out,
                                                    size_t out_size,
                                                    int32_t value_cm)
  {
      uint32_t absolute_value;
      ...
      if (value_cm < 0)
      {
          absolute_value = (uint32_t)(-value_cm);
          snprintf(out, out_size, "-%lu.%02lum",
                   (unsigned long)(absolute_value / 100u),
                   (unsigned long)(absolute_value % 100u));
      }
      else
  ...
  /* -------------------------------------------------------------------------- */
  /*  grade(0.1%) -> x.x%% helper                                               */
  /* -------------------------------------------------------------------------- */
  static void APP_FormatSignedGradeX10Text(char *out,
                                           size_t out_size,
                                           int16_t value_x10)
  {
  ...
  /*  mm/s^2 -> m/s^2 helper                                                    */
  /*                                                                            */
  /*  altitude.accel_bias_mmps2 는 1/1000 m/s^2 스케일이므로                     */
  /*  디버그 화면에서는 두 자리 소수로 잘라 표시한다.                           */
  /* -------------------------------------------------------------------------- */
  static void APP_FormatSignedMmps2AsMps2Text(char *out,
                                              size_t out_size,
                                              int32_t value_mmps2)
  {
      uint32_t absolute_value;
      uint32_t fractional_x100;
      ...
      if (value_mmps2 < 0)
      {
          absolute_value = (uint32_t)(-value_mmps2);
          fractional_x100 = (absolute_value % 1000u) / 10u;
          snprintf(out, out_size, "-%lu.%02lu",
                   (unsigned long)(absolute_value / 1000u),
                   (unsigned long)fractional_x100);
      }
      else
      {
          absolute_value = (uint32_t)value_mmps2;
  ...
  /* -------------------------------------------------------------------------- */
  /*  ALTITUDE debug page draw                                                  */
  /*                                                                            */
  /*  화면 구성                                                                 */
  /*  - 4x6 고정폭 글꼴을 사용해 화면 전체를 세로 6px 간격의 text grid로 쓴다.    */
  /*  - 좌측 상단부터 아래로 차례대로                                          */
  /*      1) 출력 모드/플래그                                                   */
  /*      2) pressure/QNH/GPS/fused/relative-home                               */
  /*      3) vario/grade/bias/IMU 자세                                          */
  /*      4) 카운터/오디오/튜닝 UI                                              */
  /*    순서로 배치한다.                                                        */
  /*  - 복잡한 그래픽 대신 "모든 병렬 값이 한 화면에 다 보이는 것" 을 우선한다. */
  /* -------------------------------------------------------------------------- */
                                        ...
                                        uint32_t now_ms)
  {
      ...
      uint8_t y;
      char line[64];
      char raw_pressure_text[16];
      char filt_pressure_text[16];
      char baro_temp_text[16];
      char qnh_manual_text[16];
      char qnh_equiv_text[16];
      char std_alt_text[16];
      char qnh_alt_text[16];
      char gps_alt_text[16];
      char abs_noimu_text[16];
      char abs_imu_text[16];
      char rel_noimu_text[16];
      char rel_imu_text[16];
      char vario_noimu_text[16];
      char vario_imu_text[16];
      char grade_noimu_text[12];
      char grade_imu_text[12];
      char bias_noimu_text[16];
      char bias_imu_text[16];
      char acc_bias_text[16];
      char roll_text[16];
      char pitch_text[16];
      char tune_value_text[32];

      (void)now_ms;

      if ((u8g2 == 0) || (alt == 0) || (settings == 0))
      {
          return;
      }

      cfg = settings->altitude;

      APP_FormatSignedCentiText(raw_pressure_text,  sizeof(raw_pressure_text),  alt->pressure_hpa_x100_raw);
      APP_FormatSignedCentiText(filt_pressure_text, sizeof(filt_pressure_text), alt->pressure_hpa_x100_filt);
      APP_FormatSignedCentiText(baro_temp_text,     sizeof(baro_temp_text),     alt->temp_cdeg_baro);
      APP_FormatSignedCentiText(qnh_manual_text,    sizeof(qnh_manual_text),    alt->qnh_manual_hpa_x100);
      APP_FormatSignedCentiText(qnh_equiv_text,     sizeof(qnh_equiv_text),     alt->qnh_equiv_gps_hpa_x100);

      APP_FormatSignedCentimeterAsMeterText(std_alt_text,     sizeof(std_alt_text),     alt->alt_pressure_std_cm);
      APP_FormatSignedCentimeterAsMeterText(qnh_alt_text,     sizeof(qnh_alt_text),     alt->alt_qnh_manual_cm);
      APP_FormatSignedCentimeterAsMeterText(gps_alt_text,     sizeof(gps_alt_text),     alt->alt_gps_hmsl_cm);
      APP_FormatSignedCentimeterAsMeterText(abs_noimu_text,   sizeof(abs_noimu_text),   alt->alt_fused_noimu_cm);
      APP_FormatSignedCentimeterAsMeterText(abs_imu_text,     sizeof(abs_imu_text),     alt->alt_fused_imu_cm);
      APP_FormatSignedCentimeterAsMeterText(rel_noimu_text,   sizeof(rel_noimu_text),   alt->alt_rel_home_noimu_cm);
      APP_FormatSignedCentimeterAsMeterText(rel_imu_text,     sizeof(rel_imu_text),     alt->alt_rel_home_imu_cm);
      APP_FormatSignedCentimeterAsMeterText(vario_noimu_text, sizeof(vario_noimu_text), alt->vario_slow_noimu_cms);
      APP_FormatSignedCentimeterAsMeterText(vario_imu_text,   sizeof(vario_imu_text),   alt->vario_slow_imu_cms);
      APP_FormatSignedCentimeterAsMeterText(bias_noimu_text,  sizeof(bias_noimu_text),  alt->baro_bias_noimu_cm);
      APP_FormatSignedCentimeterAsMeterText(bias_imu_text,    sizeof(bias_imu_text),    alt->baro_bias_imu_cm);
      APP_FormatSignedMmps2AsMps2Text(acc_bias_text, sizeof(acc_bias_text), alt->accel_bias_mmps2);
      APP_FormatSignedGradeX10Text(grade_noimu_text, sizeof(grade_noimu_text), alt->grade_noimu_x10);
      APP_FormatSignedGradeX10Text(grade_imu_text,   sizeof(grade_imu_text),   alt->grade_imu_x10);
      APP_FormatSignedCentiText(roll_text,  sizeof(roll_text),  alt->imu_roll_cdeg);
      APP_FormatSignedCentiText(pitch_text, sizeof(pitch_text), alt->imu_pitch_cdeg);

      u8g2_SetFont(u8g2, u8g2_font_4x6_tr);
      y = 6u;

      /* ---------------------------------------------------------------------- */
      /*  1) 타이틀 + 현재 출력 정책/토글 상태                                  */
      /* ---------------------------------------------------------------------- */
      APP_DrawTextLine(u8g2, &y, "ALTITUDE DBG S1/NEXT");

      snprintf(line, sizeof(line), "OUT:%s IMU:%u AUD:%u HOME:%u",
               APP_ALTITUDE_GetOutputModeText((app_alt_output_mode_t)cfg.output_alt_mode),
               cfg.use_imu_assist ? 1u : 0u,
               cfg.vario_audio_enable ? 1u : 0u,
               ((alt->flags & APP_ALT_FLAG_HOME_VALID) != 0u) ? 1u : 0u);
      APP_DrawTextLine(u8g2, &y, line);

      /* ---------------------------------------------------------------------- */
      /*  2) pressure / QNH / 절대고도 병렬 값                                  */
      /* ---------------------------------------------------------------------- */
      snprintf(line, sizeof(line), "P %s/%s T:%sC",
               raw_pressure_text,
               filt_pressure_text,
               baro_temp_text);
      APP_DrawTextLine(u8g2, &y, line);

      snprintf(line, sizeof(line), "Q %s EQ %s",
               qnh_manual_text,
               qnh_equiv_text);
      APP_DrawTextLine(u8g2, &y, line);

      snprintf(line, sizeof(line), "STD %s QNH %s",
               std_alt_text,
               qnh_alt_text);
      APP_DrawTextLine(u8g2, &y, line);

      snprintf(line, sizeof(line), "GPS %s W:%u%%",
               gps_alt_text,
               (unsigned)alt->gps_weight_pct);
      APP_DrawTextLine(u8g2, &y, line);

      snprintf(line, sizeof(line), "NI %s V %s",
               abs_noimu_text,
               vario_noimu_text);
      APP_DrawTextLine(u8g2, &y, line);

      snprintf(line, sizeof(line), "IM %s V %s",
               abs_imu_text,
               vario_imu_text);
      APP_DrawTextLine(u8g2, &y, line);

      snprintf(line, sizeof(line), "RH %s / %s",
               rel_noimu_text,
               rel_imu_text);
      APP_DrawTextLine(u8g2, &y, line);
      ...
      /*  3) grade / bias / 자세 / 카운터                                       */
      /* ---------------------------------------------------------------------- */
      snprintf(line, sizeof(line), "GR %s / %s",
               grade_noimu_text,
               grade_imu_text);
      APP_DrawTextLine(u8g2, &y, line);

      snprintf(line, sizeof(line), "BI %s/%s A%s",
               bias_noimu_text,
               bias_imu_text,
               acc_bias_text);
      APP_DrawTextLine(u8g2, &y, line);

      snprintf(line, sizeof(line), "RP %s %s T:%u%%",
               roll_text,
               pitch_text,
               (unsigned)alt->imu_trust_pct);
      APP_DrawTextLine(u8g2, &y, line);

      snprintf(line, sizeof(line), "CNT b:%lu g:%lu i:%lu",
               (unsigned long)alt->baro_sample_count,
               (unsigned long)alt->gps_sample_count,
               (unsigned long)alt->imu_sample_count);
      APP_DrawTextLine(u8g2, &y, line);

      snprintf(line, sizeof(line), "REJ bs:%lu g:%lu ig:%lu",
               (unsigned long)alt->rejected_baro_spike_count,
               (unsigned long)alt->rejected_gps_count,
               (unsigned long)alt->imu_gated_count);
      APP_DrawTextLine(u8g2, &y, line);

      snprintf(line, sizeof(line), "AUD %uHz on:%u pd:%u a:%u",
               (unsigned)alt->vario_audio_last_freq_hz,
               (unsigned)alt->vario_audio_last_on_ms,
               (unsigned)alt->vario_audio_last_period_ms,
               (unsigned)alt->vario_audio_active);
      APP_DrawTextLine(u8g2, &y, line);
      ...
      /*                                                                        */
      /*  view mode                                                              */
      /*    B2 : output mode cycle                                               */
      /*    B3 : vario audio on/off                                              */
      /*    B4 : tune mode enter                                                 */
      /*    B5 : home 기준점 재설정                                              */
      /*    B6 : primary IMU assist on/off                                       */
      /*                                                                        */
      /*  tune mode                                                              */
      /*    B2/B3 : field select                                                 */
      /*    B4/B5 : value -, +                                                   */
      /*    B6    : tune mode exit                                               */
      /* ---------------------------------------------------------------------- */
      if (g_altitude_edit.active != false)
      {
          APP_ALTITUDE_FormatTuneFieldValue(&cfg,
                                            (app_altitude_tune_field_t)g_altitude_edit.selected_field,
                                            tune_value_text,
                                            sizeof(tune_value_text));

          snprintf(line, sizeof(line), "T %u/%u %s",
                   (unsigned)(g_altitude_edit.selected_field + 1u),
                   (unsigned)APP_ALT_TUNE_COUNT,
                   APP_ALTITUDE_GetTuneFieldText((app_altitude_tune_field_t)g_altitude_edit.selected_field));
          APP_DrawTextLine(u8g2, &y, line);

          APP_DrawTextLine(u8g2, &y, tune_value_text);
          APP_DrawTextLine(u8g2, &y, "B2< B3> B4- B5+ B6 EXIT");
      }
      else
      {
          APP_DrawTextLine(u8g2, &y, "B2 MODE B3 AUDIO");
          APP_DrawTextLine(u8g2, &y, "B4 TUNE B5 HOME");
          APP_DrawTextLine(u8g2, &y, "B6 IMU PRIMARY");
      }
  }
  ...
  /* -------------------------------------------------------------------------- */
  /*  ALTITUDE debug page 전용 버튼 처리                                        */
  /*                                                                            */
  /*  view mode                                                                  */
  /*    B2 short : output altitude mode cycle                                    */
  /*    B3 short : vario audio on/off                                            */
  /*    B4 short : tune mode 진입                                                */
  /*    B5 short : relative-home 기준점 재설정                                  */
  /*    B6 short : primary output의 IMU assist on/off                            */
  /*                                                                            */
  /*  tune mode                                                                  */
  /*    B2 short : 이전 tuning field                                             */
  /*    B3 short : 다음 tuning field                                             */
  /*    B4 short : 현재 field -1 step                                            */
  /*    B5 short : 현재 field +1 step                                            */
  /*    B6 short : tune mode 종료                                                */
  /* -------------------------------------------------------------------------- */
  static bool APP_HandleAltitudePageButtonEvent(const button_event_t *event, uint32_t now_ms)
  {
      ...
      if (g_altitude_edit.active != false)
      {
          ...
          {
              if (g_altitude_edit.selected_field == 0u)
              {
                  g_altitude_edit.selected_field = (uint8_t)(APP_ALT_TUNE_COUNT - 1u);
              }
              else
              {
                  g_altitude_edit.selected_field--;
              }
              return true;
          }
          ...
          {
              g_altitude_edit.selected_field =
                  (uint8_t)((g_altitude_edit.selected_field + 1u) % (uint8_t)APP_ALT_TUNE_COUNT);
              return true;
          }
          else if ((event->id == BUTTON_ID_4) || (event->id == BUTTON_ID_5))
          {
              APP_ALTITUDE_CopySettings(&cfg);
              APP_ALTITUDE_AdjustSettingField(&cfg,
                                             (app_altitude_tune_field_t)g_altitude_edit.selected_field,
                                             (event->id == BUTTON_ID_4) ? -1 : +1);
              APP_ALTITUDE_SetSettings(&cfg);
              return true;
          }
          else if (event->id == BUTTON_ID_6)
          {
              g_altitude_edit.active = false;
              return true;
          }
      ...
      if (event->id == BUTTON_ID_2)
      {
          cfg.output_alt_mode = (uint8_t)((cfg.output_alt_mode + 1u) % (uint8_t)APP_ALT_OUTPUT_COUNT);
          APP_ALTITUDE_SetSettings(&cfg);
          return true;
      }
      else if (event->id == BUTTON_ID_3)
      {
          cfg.vario_audio_enable = (cfg.vario_audio_enable == 0u) ? 1u : 0u;
          APP_ALTITUDE_SetSettings(&cfg);
          return true;
      }
      else if (event->id == BUTTON_ID_4)
      {
          g_altitude_edit.active = true;
          if (g_altitude_edit.selected_field >= (uint8_t)APP_ALT_TUNE_COUNT)
          {
              g_altitude_edit.selected_field = 0u;
          }
          return true;
      }
      else if (event->id == BUTTON_ID_5)
      {
          APP_ALTITUDE_ResetHomeNow(now_ms);
          return true;
      }
      else if (event->id == BUTTON_ID_6)
      {
          cfg.use_imu_assist = (cfg.use_imu_assist == 0u) ? 1u : 0u;
          APP_ALTITUDE_SetSettings(&cfg);
          return true;
      }
  ...
  static void APP_HandleButtonEvents(void)
  {
      ...
      while (Button_PopEvent(&event) != false)
      {
          ...
          /* ------------------------------------------------------------------ */
          /*  ALTITUDE / CLOCK test 페이지 전용 버튼 매핑은                      */
          /*  page별 helper에서 먼저 처리한다.                                   */
          /* ------------------------------------------------------------------ */
...
void UI_DebugLegacy_Init(void)
{
    ...
    memset(&g_altitude_edit, 0, sizeof(g_altitude_edit));
