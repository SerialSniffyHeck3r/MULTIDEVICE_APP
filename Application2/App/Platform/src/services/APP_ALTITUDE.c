                                                      ...
                                                      bool *out_vector_valid)
{
    ...
    accel_alpha   = APP_ALTITUDE_LpfAlphaFromTauMs(settings->imu_accel_tau_ms, dt_s);
...
static void APP_ALTITUDE_HandleDebugAudio(uint32_t now_ms, const app_altitude_settings_t *settings)
{
    ...
    vario_cms = s_altitude_runtime.vario_fast_noimu_cms;
    ...
    period_ms = settings->audio_repeat_ms;
    if (vario_cms > 0.0f)
    {
        period_ms = (uint32_t)((float)period_ms * (1.0f - 0.45f * scale));
    }
    else
    {
        period_ms = (uint32_t)((float)period_ms * (1.15f - 0.20f * scale));
    }
    period_ms = APP_ALTITUDE_ClampU32(period_ms, 40u, 1000u);
    ...
    if (vario_cms >= 0.0f)
    {
        freq_hz = (uint32_t)((float)settings->audio_min_freq_hz +
                  ((float)(settings->audio_max_freq_hz - settings->audio_min_freq_hz) * scale));
        status = Audio_Driver_PlaySquareWaveMs(freq_hz, settings->audio_beep_ms);
    }
    else
    {
        freq_hz = (uint32_t)((float)settings->audio_min_freq_hz * (0.55f + 0.30f * scale));
        status = Audio_Driver_PlaySawToothWaveMs(freq_hz, settings->audio_beep_ms);
    }
...
void APP_ALTITUDE_Task(uint32_t now_ms)
{
    ...
    settings = &g_app_state.settings.altitude;
    baro     = &g_app_state.gy86.baro;
    gps_fix  = &g_app_state.gps.fix;
    mpu      = &g_app_state.gy86.mpu;
    ...
    if ((new_baro_sample != false) && (s_altitude_runtime.kf_noimu.valid != false))
    {
        APP_ALTITUDE_Kf3_UpdateScalar(s_altitude_runtime.kf_noimu.x,
                                      ...
                                      H_baro3,
                                      alt_qnh_cm,
                                      APP_ALTITUDE_SquareF((float)settings->baro_measurement_noise_cm));
    }
