...
static void uc1608_init_smart_update_state(void)
{
    ...
    for (uint16_t i = 0u; i < s_uc1608_buffer_size; ++i)
    {
...
void U8G2_UC1608_CommitBuffer(void)
{
    ...
    if (g_u8g2_uc1608_runtime.smart_update_enable == 0u)
    {
        u8g2_SendBuffer(&g_u8g2);
        return;
    }
    ...
    tile_width  = u8g2_GetBufferTileWidth(&g_u8g2);
    tile_height = u8g2_GetBufferTileHeight(&g_u8g2);
    tile_count  = (uint16_t)tile_width * (uint16_t)tile_height;

    {
        uint8_t tx_min = tile_width;
        ...
        u8g2_UpdateDisplayArea(&g_u8g2,
                               tx_min,
                               ty_min,
                               (uint8_t)(tx_max - tx_min + 1u),
                               (uint8_t)(ty_max - ty_min + 1u));
    }
}
