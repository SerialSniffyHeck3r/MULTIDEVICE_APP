  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  uint32_t now_ms;
	        ...
	        /* ------------------------------------------------------------------ */
	        /* Soft Power 상태머신 주기 처리                                      */
	        /*                                                                    */
	        /* 수행 내용                                                          */
	        /* - Soft Power PUSH pin polling + debounce                           */
	        /* - short press -> QUICK SETTINGS STUB                               */
	        /* - long press  -> POWER OFF confirm fullscreen                      */
	        /* - blocking overlay 상태일 때 F1/F6 이벤트 소비 및 자체 draw        */
	        /* ------------------------------------------------------------------ */
	        POWER_STATE_Task(now_ms);

	        /* ------------------------------------------------------------------ */
	        /* 전원 오버레이가 LCD를 점유 중이 아닐 때만                           */
	        /* 기존 UI 엔진을 실행한다.                                            */
	        /*                                                                    */
	        /* 이렇게 해야 QUICK SETTINGS / POWER OFF 확인 화면 위에               */
	        /* 기존 화면이 다시 덧그려지지 않는다.                                 */
	        /* ------------------------------------------------------------------ */
