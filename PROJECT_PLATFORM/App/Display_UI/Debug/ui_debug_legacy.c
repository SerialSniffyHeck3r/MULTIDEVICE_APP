...
/* -------------------------------------------------------------------------- */
  ...
  typedef enum
  {
      APP_UI_PAGE_SENSOR      = 0u,
      APP_UI_PAGE_BRIGHTNESS  = 1u,
      APP_UI_PAGE_SD          = 2u,
      APP_UI_PAGE_BLUETOOTH   = 3u,
      APP_UI_PAGE_AUDIO       = 4u,
      APP_UI_PAGE_SPI_FLASH   = 5u,
      APP_UI_PAGE_CLOCK       = 6u,
      APP_UI_PAGE_ALTITUDE    = 7u,
      APP_UI_PAGE_BARO_FUSION = 8u,
      APP_UI_PAGE_BIKE        = 9u,
  ...
  static const char *APP_GetSdFsStageText(uint8_t stage)
  {
      switch (stage)
      {
      case APP_SD_FS_STAGE_NONE: return "--";
      case APP_SD_FS_STAGE_MOTOR_REC_DIR_STAT: return "DSTAT";
      case APP_SD_FS_STAGE_MOTOR_REC_DIR_MKDIR: return "DMK";
      case APP_SD_FS_STAGE_MOTOR_REC_NAME_OPENDIR: return "NDIR";
      case APP_SD_FS_STAGE_MOTOR_REC_NAME_READDIR: return "NREAD";
      case APP_SD_FS_STAGE_MOTOR_REC_OPEN: return "OPEN";
      case APP_SD_FS_STAGE_MOTOR_REC_WRITE: return "WRITE";
      case APP_SD_FS_STAGE_MOTOR_REC_SYNC: return "SYNC";
      case APP_SD_FS_STAGE_MOTOR_REC_CLOSE: return "CLOSE";
      default: return "RAW";
      }
  }

  static const char *APP_GetSdIoOpText(uint8_t io_op)
  {
      switch (io_op)
      {
      case APP_SD_IO_OP_NONE: return "--";
      case APP_SD_IO_OP_READ: return "RD";
      case APP_SD_IO_OP_WRITE: return "WR";
      default: return "RAW";
      }
  }
                                  ...
                                  uint32_t now_ms)
  {
      ...
      snprintf(line, sizeof(line), "FS %s:%s",
               APP_GetSdFsStageText(sd->last_fs_stage),
               APP_GetSdFresultText(sd->last_fs_fresult));
      APP_DrawTextLine(u8g2, &y, line);

      snprintf(line, sizeof(line), "IO %s b:%u h:%u tr:%s",
               APP_GetSdIoOpText(sd->last_io_op),
               (unsigned)sd->last_io_bsp_status,
               (unsigned)sd->last_io_hal_status,
               APP_GetSdTransferStateText(sd->last_io_transfer_state));
      APP_DrawTextLine(u8g2, &y, line);

      snprintf(line, sizeof(line), "BLK %lu+%lu F:%02X",
               (unsigned long)sd->last_io_block_addr,
               (unsigned long)sd->last_io_block_count,
               (unsigned)sd->last_io_buffer_flags);
      APP_DrawTextLine(u8g2, &y, line);
  ...
  static volatile app_ui_page_t      g_ui_page = APP_UI_PAGE_SENSOR;
  ...
  static void APP_SelectNextUiPage(void)
  {
      ...
      {
          next_page_index = (uint32_t)APP_UI_PAGE_SENSOR;
      }
