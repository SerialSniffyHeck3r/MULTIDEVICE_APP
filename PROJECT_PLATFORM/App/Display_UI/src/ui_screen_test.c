                                                        ...
                                                        uint32_t now_ms)
{
  ...
  /* ---------------------------------------------------------------------- */
   ...
   /*                                                                        */
   /* F6 short press는 이제 U8G2 FONT TEST 브라우저의                         */
   /* "다음 폰트" 기능으로 재할당된다.                                        */
   /* long press의 GPS 진입 action은 그대로 유지한다.                         */
   /* ---------------------------------------------------------------------- */
...
/* -------------------------------------------------------------------------- */
/* Local helper: font preview selector                                        */
/* -------------------------------------------------------------------------- */
static const ui_test_font_preview_t *ui_screen_test_get_font_preview(uint8_t index)
{
  if (UI_TEST_FONT_PREVIEW_COUNT == 0u)
  {
    return 0;
  }
...
/* -------------------------------------------------------------------------- */
/* Local helper: current font line height                                     */
/* -------------------------------------------------------------------------- */
static int16_t ui_screen_test_measure_font_line_height(u8g2_t *u8g2)
{
  int16_t ascent;
  ...
  line_h = (int16_t)(ascent + descent + 1);
  if (line_h < 6)
  {
    line_h = 6;
  }
...
/* -------------------------------------------------------------------------- */
/* Local helper: centered text draw                                           */
/* -------------------------------------------------------------------------- */
static void ui_screen_test_draw_centered_text(u8g2_t *u8g2,
                                              const ui_rect_t *rect,
                                              int16_t baseline_y,
