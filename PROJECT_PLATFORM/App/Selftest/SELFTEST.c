...
/* ?깆뿉???대? 怨듦컻?섍퀬 ?덈뒗 ?곸쐞 ?덈꺼 ?곹깭/?쒕씪?대쾭 API留??ъ슜?쒕떎. */
#include "APP_STATE.h"
#include "APP_SD.h"
#include "Ublox_GPS.h"
#include "GY86_IMU.h"
...
/*                                                                            */
/* ?쇰? ?뚯씪 諛곗튂?먯꽌??APP_STATE.h ?대? ?좎뼵??include ?쒖꽌??誘쇨컧?????덉뼱  */
/* ?ш린???숈씪 ?쒓렇?덉쿂瑜???踰????좎뼵?쒕떎.                                  */
/*                                                                            */
/* ???좎뼵? "??low-level ?⑥닔 異붽?"媛 ?꾨땲??                               */
/* ?대? 湲곗〈 quick self-test媛 ?ъ슜?섎뜕 怨듦컻 snapshot API瑜?洹몃?濡??ъ궗?⑺븯??*/
/* 紐⑹쟻?대떎.                                                                   */
/* -------------------------------------------------------------------------- */
...
/* 濡쒖뺄 ?곸닔                                                                   */
/*                                                                            */
/* Boot/APP 二쇱냼??bootloader 履?FW_BootConfig.h 怨꾩빟怨?留욎텣??               */
/* - bootloader : 0x08000000 ~ 0x0801FFFF (128 KB)                            */
/* - main app   : 0x08020000 ~ flash end                                      */
/*                                                                            */
/* app 履??꾨줈?앺듃?먯꽌??bootloader ?ㅻ뜑瑜?吏곸젒 include ?섏? ?딄퀬,             */
/* ?좎?蹂댁닔 self-test ?꾩슜 ?쎄린 ?곸닔濡쒕쭔 以묐났 ?뺤쓽?쒕떎.                        */
/* -------------------------------------------------------------------------- */
...
/* STM32F407VG 怨꾩뿴??RAM 踰붿쐞瑜?蹂댁닔?곸쑝濡??〓뒗??
 * - SRAM1/2 : 0x20000000 ~ 0x2002FFFF (192 KB 踰붿쐞 ?ъ슜)
...
/* 硫붾돱 ID                                                                     */
/*                                                                            */
/* ?レ옄 ?쒖떆李?bottom center)? 2-digit zero padded ?뺤떇?쇰줈 蹂댁뿬 二쇰?濡?     */
/* ?ㅼ젣 ID??00 ~ 10 援ъ“濡?留욎텣??                                           */
/* -------------------------------------------------------------------------- */
typedef enum
{
...
/* -------------------------------------------------------------------------- */
/* ?좎?蹂댁닔 紐⑤뱶 ?꾩슜 scratch RAM                                              */
/*                                                                            */
/* full destructive March test??live app 臾몃㎘?먯꽌???쒖뒪???먯껜瑜?源⑤쑉由???  */
/* ?덉쑝誘濡? ?ш린?쒕뒗 dedicated scratch buffer?????non-destructive pattern  */
/* verify瑜??섑뻾?쒕떎.                                                          */
/* -------------------------------------------------------------------------- */
static uint8_t g_selftest_ram_scratch[SELFTEST_RAM_SCRATCH_SIZE] APP_CCMRAM_BSS;
...
/* CRC32                                                                       */
/*                                                                            */
/* build-time golden CRC manifest媛 ?꾩옱 ?꾨줈?앺듃???놁쑝誘濡?                  */
/* ?ш린?쒕뒗 "怨꾩궛 ?먯껜???섑뻾"?섍퀬, 寃곌낵媛믪쓣 ?붾㈃/濡쒓렇???몄텧?쒕떎.            */
/* pass/fail? vector sanity + programmed span 議댁옱 ?щ?瑜?湲곗??쇰줈 ?쒕떎.      */
/* -------------------------------------------------------------------------- */
                                                      ...
                                                      int box_h)
{
  ...
  /* ---------------------------------------------------------------------- */
  /* ?붿껌??諛곌꼍 ?⑦꽩                                                        */
  /* - 泥?踰덉㎏ Y ?쇱씤(y=0)? ???X ?쎌?留??먮벑                              */
  /* - ??踰덉㎏ Y ?쇱씤(y=1)? 吏앹닔 X ?쎌?留??먮벑                              */
  /* - 洹??ㅼ쓬 以꾨??곕룄 ?숈씪?섍쾶 ??以꾩뵫 踰덇컝??媛硫?諛섎났                    */
  /*                                                                        */
  /* 利? 1?쇱씤 ?⑥쐞 泥댁빱蹂대뱶媛 ?꾨땲??"以꾨쭏??李띾뒗 ?쒖옉 X媛 踰덇컝??諛붾뚮뒗"   */
  /* 留ㅼ슦 ?⑥닚??service pattern?대떎.                                        */
  /* ---------------------------------------------------------------------- */
  for (y = 0; y < disp_h; ++y)
  {
...
static void SELFTEST_DrawBaseFrame(u8g2_t *u8g2)
{
  ...
  /* 硫붾돱 諛뺤뒪 ?대???源⑤걮????諛뷀깢?쇰줈 鍮꾩슫 ???꾨젅?꾨쭔 洹몃┛?? */
...
static void SELFTEST_ServiceRuntime(void)
{
  ...
  /* maintenance mode?먯꽌??湲곗〈 bring-up ?쒕퉬?ㅻ뱾???댁븘 ?덇쾶 ?좎??쒕떎.
   * - SD hotplug/mount ?곹깭 ?좎?
   * - GPS packet parser keep-alive
   * - BT RX/TX keep-alive
   * - IMU polling ?좎?
   * - ?ㅻ뵒??LED/諛깅씪?댄듃 ?뚯뒪???숈옉 ?좎?
   * - SPI flash request state machine 吏꾪뻾
   */
  APP_SD_Task(now_ms);
                                  ...
                                  selftest_exec_result_t *out_result)
{
  ...
  /* 鍮꾩묠??write-path test                                                  */
  /*                                                                        */
  /* - 湲곗〈 request state machine 湲곕컲 test???꾩옱 ?λ퉬?먯꽌 false FAIL??     */
  /*   諛섎났?쒕떎怨?蹂닿퀬?섏뿀?쇰?濡? maintenance mode?먯꽌???쒕퉬??愿?먯쓽        */
  /*   blocking helper API 寃쎈줈瑜?吏곸젒 ?ъ슜?쒕떎.                             */
  /* - ?? "湲곗〈 ?댁슜???ㅻⅨ ?⑦꽩?쇰줈 ??뼱?곌린" ???섏? ?딅뒗??               */
  /* - ?쎌? 諛붿씠?몃? 媛숈? 二쇱냼??洹몃?濡??ㅼ떆 program ?섍퀬 readback 鍮꾧탳留?   */
  /*   ?섑뻾?섏뿬, erase ?녿뒗 non-invasive write path 寃利앸쭔 ?섑뻾?쒕떎.         */
  /* ---------------------------------------------------------------------- */
                              ...
                              selftest_exec_result_t *out_result)
{
  ...
  uint32_t slot0_sample_count = 0u;
  int32_t slot0_pa = 0;
  int32_t slot1_pa = 0;
  int32_t delta_pa = 0;
  uint32_t idx;
  ...
  char slot0_valid = 'N';
  char slot0_fresh = 'N';
  char slot0_selected = 'N';
  char slot0_rejected = 'N';
  char line1[96];
  char line2[96];
  char line3[96];
  ...
  slot0_online = (imu_now.baro_sensor[0].online != 0u) ? 'Y' : 'N';
  slot0_valid = (imu_now.baro_sensor[0].valid != 0u) ? 'Y' : 'N';
  slot0_fresh = (imu_now.baro_sensor[0].fresh != 0u) ? 'Y' : 'N';
  slot0_selected = (imu_now.baro_sensor[0].selected != 0u) ? 'Y' : 'N';
  slot0_rejected = (imu_now.baro_sensor[0].rejected != 0u) ? 'Y' : 'N';
  slot0_sample_count = imu_now.baro_sensor[0].sample_count;
  ...
  (void)snprintf(line2,
                 ...
                 slot0_selected,
                 slot0_rejected,
                 (unsigned long)slot0_sample_count);
  (void)snprintf(line3,
                 sizeof(line3),
                 "p=%ld t=%ld st=%02X hs=%u err=%lu",
                 (long)imu_now.baro_sensor[0].pressure_pa,
                 (long)imu_now.baro_sensor[0].temp_cdeg,
                 (unsigned int)imu_now.debug.ms5611_state,
                 (unsigned int)imu_now.debug.last_hal_status_baro,
                 (unsigned long)imu_now.debug.baro_error_count);
                                      ...
                                      selftest_exec_result_t *out_result)
{
  ...
  /* duration_ms???쇰? guided helper?먯꽌 HAL_GetTick() raw 媛믪쑝濡??ｌ? ?딅룄濡?   * ?ш린????踰??뺣━?쒕떎. */
...
static void SELFTEST_RunGuidedComposite(selftest_exec_result_t *out_result)
{
  ...
  for (sub_id = 0u; sub_id < (uint8_t)SELFTEST_GUIDED_ITEM_COUNT; ++sub_id)
  {
    ...
    /* auto full test ?꾩쨷?먮뒗 媛?guided sub-step 寃곌낵瑜??좉퉸 蹂댁뿬以?? */
...
static void SELFTEST_RunAutoFull(selftest_exec_result_t *out_result)
{
       ...
       ++item_id)
  {
    ...
    /* 媛??뚯뒪?멸? ?앸궇 ??operator媛 利됱떆 ?깃났/?ㅽ뙣瑜?蹂????덇쾶 ?좉퉸 ?쒖떆 */
...
static void SELFTEST_MainMenuLoop(void)
{
  ...
  /* ?좎?蹂댁닔 紐⑤뱶???ㅼ젣濡??ㅼ뼱?붿쑝誘濡?
   * ?댁젣 bootloader ?낆옣?먯꽌??"?뺤긽 遺?낆씠 ?앸궃 ??maintenance loop濡?吏꾩엯?덈떎"
   * ?쇨퀬 蹂대뒗 ?몄씠 ???덉쟾?섎떎.
   */
    ...
    for (;;)
    {
      ...
      if (main_selection == (uint8_t)SELFTEST_ITEM_GUIDED)
      {
        ...
        while (leave_guided_menu == false)
        {
          ...
          /* sub-item ?ㅽ뻾 吏곹썑?먮뒗 ?ㅼ쓬 redraw瑜?利됱떆 ?덉슜?쒕떎. */
          last_guided_redraw_ms = 0u;
        }

        /* guided submenu瑜?鍮좎졇?섏삤硫?main menu??利됱떆 redraw?쒕떎. */
        last_main_redraw_ms = 0u;
        continue;
      }
...
{
  /* ---------------------------------------------------------------------- */
  /* ???⑥닔???섎룄?곸쑝濡?return ?섏? ?딅뒗??                               */
  /* - 遺??F2 hold濡?maintenance mode???ㅼ뼱?ㅻ㈃ ?쇰컲 ???덊띁猷⑦봽濡?蹂듦??섏? */
  /*   ?딄퀬, ?좎?蹂댁닔 ?꾩슜 猷⑦봽瑜??낅┰?곸쑝濡??좎??쒕떎.                       */
  /* - watchdog? FW_AppGuard_Kick()濡?怨꾩냽 refresh ?쒕떎.                    */
  /* ---------------------------------------------------------------------- */
  SELFTEST_MainMenuLoop();

  /* ?쇰━???꾨떖?섏? ?딅뒗 援ш컙.
   * ?뱀떆?쇰룄 鍮좎졇?섏삤硫??덉쟾?섍쾶 臾댄븳 猷⑦봽?먯꽌 watchdog reset??湲곕떎由곕떎. */
  for (;;)
  {
    SELFTEST_ServiceRuntime();
  }
