...
static int32_t BMP581_SignExtend24(uint32_t raw_u24)
{
    int32_t raw_s24;

    raw_s24 = (int32_t)(raw_u24 & 0x00FFFFFFu);
    if ((raw_s24 & 0x00800000L) != 0)
    {
        raw_s24 |= (int32_t)0xFF000000L;
    }
...
int32_t BMP581_Init(bmp581_device_t *dev)
{
    ...
    /*  2) 소프트 리셋                                                          */
    /* ---------------------------------------------------------------------- */
    if (BMP581_ReadRegU8(dev, BMP581_REG_STATUS, &dev->last_status) != 0)
    {
        return -1;
    }
    ...
    if ((dev->last_status & (BMP581_STATUS_NVM_ERR | BMP581_STATUS_NVM_CMD_ERR)) != 0u)
    {
        return -1;
    }

    if (BMP581_ReadRegU8(dev, BMP581_REG_CHIP_STATUS, &dev->last_chip_status) != 0)
    {
        return -1;
    }
    ...
    /* ---------------------------------------------------------------------- */
    if (BMP581_ReadReg(dev, BMP581_REG_DSP_CONFIG, reg_pair, 2u) != 0)
    {
        return -1;
    }

    reg_pair[1] = (uint8_t)((reg_pair[1] & (uint8_t)~0x3Fu) |
                            ((dev->cfg.iir_temp  & 0x07u) << BMP581_TEMP_IIR_SHIFT) |
    ...
    /* ---------------------------------------------------------------------- */
    reg = BMP581_INT_SOURCE_DRDY_DATA_REG_EN;
    if (BMP581_WriteRegU8(dev, BMP581_REG_INT_SOURCE, reg) != 0)
    {
        return -1;
    }

    if (BMP581_ReadReg(dev, BMP581_REG_OSR_CONFIG, reg_pair, 2u) != 0)
    {
        return -1;
    }

    reg_pair[0] = (uint8_t)((reg_pair[0] & (uint8_t)~0x7Fu) |
                            ((dev->cfg.osr_temp  & 0x07u) << BMP581_TEMP_OS_SHIFT) |
                            ((dev->cfg.osr_press & 0x07u) << BMP581_PRESS_OS_SHIFT) |
                            BMP581_PRESS_ENABLE_MASK);

    reg_pair[1] = (uint8_t)((reg_pair[1] & (uint8_t)~(BMP581_DEEP_DISABLE_MASK | 0x7Cu | BMP581_POWERMODE_MASK)) |
                            BMP581_DEEP_DISABLE_MASK |
                          ...
                          uint8_t *new_sample)
{
    ...
    raw_temp_s24 = BMP581_SignExtend24(raw_temp_u24);

    /* Pressure: Bosch SensorAPI path uses unsigned raw pressure. */
