...
#if (GY86_ACCELGYRO_BACKEND == GY86_ACCELGYRO_BACKEND_LSM6DSOX)
#include "LSM6DSOX.h"
#endif
...
extern I2C_HandleTypeDef GY86_IMU_I2C_HANDLE;
#if ((USE_DOUBLE_BAROSENSOR == 2u) || (GY86_BARO_BUS_ID == 2u))
extern I2C_HandleTypeDef GY86_IMU_I2C2_HANDLE;
#endif
...
typedef struct
{
    ...
    uint32_t next_baro_ms;              /* 다음 baro polling 시각                 */
...
static int32_t GY86_LsmReadCb(void *ctx,
                              uint8_t dev_addr,
                              uint8_t reg_addr,
                              uint8_t *data,
                              uint16_t len)
{
    const gy86_i2c_bus_desc_t *bus = (const gy86_i2c_bus_desc_t *)ctx;

    if (GY86_I2C_Read_Bus(bus, dev_addr, reg_addr, data, len) != HAL_OK)
    {
        return -1;
    }
...
static int32_t GY86_LsmWriteCb(void *ctx,
                               uint8_t dev_addr,
                               uint8_t reg_addr,
                               const uint8_t *data,
                               uint16_t len)
...
static void GY86_LsmDelayMsCb(void *ctx, uint32_t delay_ms)
{
    (void)ctx;
    HAL_Delay(delay_ms);
}
...
static HAL_StatusTypeDef GY86_Lsm6dsox_Init(app_gy86_state_t *imu)
{
    ...
    st = HAL_I2C_IsDeviceReady(s_gy86_i2c_bus1.handle,
                               GY86_ALT_LSM6DSOX_ADDR,
                               2u,
                               GY86_IMU_I2C_TIMEOUT_MS);
    imu->debug.last_hal_status_mpu = (uint8_t)st;
    if (st != HAL_OK)
    {
        GY86_I2C_RecoverBus(&s_gy86_i2c_bus1);
        st = HAL_I2C_IsDeviceReady(s_gy86_i2c_bus1.handle,
                                   GY86_ALT_LSM6DSOX_ADDR,
                                   2u,
                                   GY86_IMU_I2C_TIMEOUT_MS);
    ...
    memset(&s_gy86_rt.lsm_dev, 0, sizeof(s_gy86_rt.lsm_dev));
    s_gy86_rt.lsm_dev.io.read     = GY86_LsmReadCb;
    s_gy86_rt.lsm_dev.io.write    = GY86_LsmWriteCb;
    s_gy86_rt.lsm_dev.io.delay_ms = GY86_LsmDelayMsCb;
    s_gy86_rt.lsm_dev.io.ctx      = (void *)&s_gy86_i2c_bus1;
...
#if (GY86_BARO_BACKEND == GY86_BARO_BACKEND_BMP581)
static const gy86_i2c_bus_desc_t *GY86_Bmp581_GetBus(void)
{
#if (GY86_BARO_BUS_ID == 2u)
    return &s_gy86_i2c_bus2;
#else
...
static HAL_StatusTypeDef GY86_Bmp581_IsReady(const gy86_i2c_bus_desc_t *bus,
                                             uint8_t dev_addr)
{
    HAL_StatusTypeDef st;
...
static int32_t GY86_Bmp581_ReadCb(void *ctx,
                                  uint8_t dev_addr,
                                  uint8_t reg_addr,
                                  uint8_t *data,
                                  uint16_t len)
...
static int32_t GY86_Bmp581_WriteCb(void *ctx,
                                   uint8_t dev_addr,
                                   uint8_t reg_addr,
                                   const uint8_t *data,
                                   uint16_t len)
...
static void GY86_Bmp581_DelayMsCb(void *ctx, uint32_t delay_ms)
{
    (void)ctx;
    HAL_Delay(delay_ms);
}

static void GY86_Bmp581_ClearBaroSensorDiagnostics(app_gy86_state_t *imu)
{
    if (imu == 0)
    {
        return;
    ...
    imu->baro_sensor[0].configured   = 1u;
    imu->baro_sensor[0].online       = s_gy86_rt.baro_online;
    imu->baro_sensor[0].bus_id       = GY86_BARO_BUS_ID;
    imu->baro_sensor[0].addr_7bit    = (uint8_t)(GY86_ALT_BMP581_ADDR >> 1);
    imu->baro_sensor[0].error_streak = s_gy86_rt.baro_error_streak;
...
static HAL_StatusTypeDef GY86_Bmp581_Init(app_gy86_state_t *imu)
{
    ...
    if (GY86_Bmp581_IsReady(bus, GY86_ALT_BMP581_ADDR) != HAL_OK)
    {
        imu->debug.last_hal_status_baro = (uint8_t)HAL_ERROR;
        return HAL_ERROR;
    }

    memset(&s_gy86_rt.bmp581_dev, 0, sizeof(s_gy86_rt.bmp581_dev));
    s_gy86_rt.bmp581_dev.io.read     = GY86_Bmp581_ReadCb;
    s_gy86_rt.bmp581_dev.io.write    = GY86_Bmp581_WriteCb;
    s_gy86_rt.bmp581_dev.io.delay_ms = GY86_Bmp581_DelayMsCb;
    s_gy86_rt.bmp581_dev.io.ctx      = (void *)bus;

    s_gy86_rt.bmp581_dev.cfg.i2c_addr            = GY86_ALT_BMP581_ADDR;
    s_gy86_rt.bmp581_dev.cfg.osr_temp            = GY86_ALT_BMP581_TEMP_OSR;
    s_gy86_rt.bmp581_dev.cfg.osr_press           = GY86_ALT_BMP581_PRESS_OSR;
    s_gy86_rt.bmp581_dev.cfg.odr                 = GY86_ALT_BMP581_ODR;
    s_gy86_rt.bmp581_dev.cfg.iir_temp            = GY86_ALT_BMP581_IIR_TEMP;
    s_gy86_rt.bmp581_dev.cfg.iir_press           = GY86_ALT_BMP581_IIR_PRESS;
    s_gy86_rt.bmp581_dev.cfg.accept_chip_id_0x51 = GY86_ALT_BMP581_ACCEPT_CHIP_ID_0x51;

    if (BMP581_Init(&s_gy86_rt.bmp581_dev) != 0)
    {
        imu->debug.last_hal_status_baro = (uint8_t)HAL_ERROR;
        imu->debug.ms5611_state         = s_gy86_rt.bmp581_dev.last_status;
        return HAL_ERROR;
    }
    ...
    GY86_Bmp581_ClearBaroSensorDiagnostics(imu);
    imu->baro_sensor[0].online = 1u;
                                          ...
                                          uint8_t *updated)
{
    ...
    memset(&sample, 0, sizeof(sample));
    if (BMP581_ReadSample(&s_gy86_rt.bmp581_dev, &sample, &new_sample) != 0)
    {
        imu->debug.last_hal_status_baro = (uint8_t)HAL_ERROR;
        imu->debug.baro_error_count++;
        imu->baro_sensor[0].error_streak = (uint8_t)(s_gy86_rt.baro_error_streak + 1u);
...
static void GY86_ResetAppStateSlice(app_gy86_state_t *imu)
{
    ...
    imu->debug.baro_poll_period_ms = GY86_IMU_ACTIVE_BARO_PERIOD_MS;
    imu->debug.baro_device_slots = APP_GY86_BARO_SENSOR_SLOTS;
    imu->debug.baro_primary_sensor_index = 0xFFu;
...
static void GY86_UpdateInitializedFlag(app_gy86_state_t *imu)
{
    ...
    /* ---------------------------------------------------------------------- */
#if (GY86_BARO_BACKEND == GY86_BARO_BACKEND_BMP581)
    /* BMP581 path uses a single backend-level online flag. */
#else
    s_gy86_rt.baro_online = GY86_Ms5611_AnyOnline();
#endif
...
#if (GY86_BARO_BACKEND == GY86_BARO_BACKEND_BMP581)
        s_gy86_rt.next_baro_ms = GY86_NextPeriodicDue(s_gy86_rt.next_baro_ms,
                                                      GY86_IMU_ACTIVE_BARO_PERIOD_MS,
                                                      now_ms);
#endif
    }
}
